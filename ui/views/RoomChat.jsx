...
export default function RoomChat({room}) {
  ...
  let [chatScrollPosition, setChatScrollPosition] = useState(
    sessionStorage.getItem(`${roomId}.chatScrollPosition`) ?? -999
  );
  ...
  function processText(text) {
    ...
    return text.replace(urlRegex, match => {
      ...
      const fullUrlTag = queryString
        ? `<a href="${url}${queryString}" target="_blank" style="font-weight:300; text-decoration:underline;">${queryString}</a>`
        : '';
  ...
  return (
    <div
      id="roomChat"
      className="max-h-96 h-full w-full bg-gray-700"
    ...
    >
      ...
      >
        {textchats.map((textentry, index) => {
          ...
          return (
            ...
            >
              <img
                className="flex w-6 h-6 human-radius cursor-pointer"
                src={useravatar}
              />
              <div className="flex mx-2 text-sm font-bold">{username}</div>
              <div
                className="flex-grow text-sm"
                dangerouslySetInnerHTML={{__html: processText(thetext)}}
              />
            </div>
          );
        })}
      ...
      <div
        className="flex w-full justify-between my-3 absolute mb-0"
