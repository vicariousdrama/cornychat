...
const {adFileLocationPath2} = require('../config');

const router = express.Router({mergeParams: true});

router.get('', async function (req, res) {
    const roomId = req.params.id;
    const adIndex = req.params.idx ?? 0;

    const d = new Date();
    const y = String(d.getUTCFullYear());
    const m = ("00" + String(d.getUTCMonth() + 1)).slice(-2);
    const adFolder = path.join(adFileLocationPath2, y, m);
    ...
    let adfile = path.join(adFileLocationPath2, "ad320x50.png");
    if(fileCount > 0) {
        let imgnum = (Math.floor(Date.now()/1000) % fileCount);
        if (fileCount > adIndex) imgnum = adIndex;
