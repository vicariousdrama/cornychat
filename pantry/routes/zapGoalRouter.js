...
const {serverZapGoalUpdateInterval} = require('../config');
...
router.get('', async function (req, res) {
  ...
  if (pk.length == 0) {
    ...
    // if server zap goals disabled, alter the response
    if (serverZapGoalUpdateInterval <= 0) {
      for (let t of theZapgoal.tags) {
        if (t.length > 1 && t[0] == 'amount') t[1] = 0;
      }
    }
    res.send(theZapgoal);
    return;
