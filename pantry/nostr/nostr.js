...
const deleteNostrSchedule = async (roomId, oldScheduledStart) => {
  try {
    ...
    if (pmd)
      console.log(
        '[deleteNostrSchedule] removing schedule for room ',
        roomId,
        ', old start: ',
        oldScheduledStart
      );
    ...
    console.log(
      `[deleteNostrSchedule] updating event ${aTagValue} of room ${roomId} as no longer scheduled`
    );
    ...
    let ejectedEntry = await ejectServerSchedule({
      startTime: oldScheduledStart,
      location: `https://${jamHost}/${roomId}`,
    });
    ...
    try {
      ...
      try {
        ...
        const calendarFilter = [
          {kinds: [31923], limit: 5000, since: daysago30},
        ];
        const deleteFilter = [{kinds: [5], limit: 5000, since: daysago30}];
        const liveactivitiesFilter = [
          {kinds: [30311], limit: 5000, since: daysago1},
        ];
        ...
        setTimeout(() => {
          ...
          console.log(
            '[getScheduledEvents] number of audiospace calendar events: ',
          ...
          for (let event of calendarEvents) {
            ...
            for (let eventTag of eventTags) {
              ...
              if (eventTag[0] == 't' && eventTag[1] == 'audiospace') {
                isAudioSpace = true;
              } // backwards compatible, can remove after 20240401
              if (eventTag[0] == 'l' && eventTag[1] == 'audiospace') {
                isAudioSpace = true;
              }
              if (eventTag[0] == 'location' && eventTag[1].length > 0) {
                location = eventTag[1];
              }
              if (eventTag[0] == 'title' && eventTag[1].length > 0) {
                title = eventTag[1];
              }
              if (eventTag[0] == 'image' && eventTag[1].length > 0) {
                image = eventTag[1];
              }
              if (eventTag[0] == 'deleted' && eventTag[1].length > 0) {
                isDeleted = true;
              } // backwards compatible, can remove after 20240401
              ...
              if (eventTag[0] == 'd' && eventTag[1].length > 0) {
                dTag = eventTag[1];
              }
            }
            // Check for deleted
            ...
            if (startTime == undefined) {
              if (pmd)
                console.log(
                  '[getScheduledEvents] skipping event that has no start time'
                );
            ...
            if (endTime == undefined) {
              if (pmd)
                console.log(
                  '[getScheduledEvents] skipping event that has no end time'
                );
              continue; // must have a time
            }
            if (startTime > endTime) {
              if (pmd)
                console.log(
                  '[getScheduledEvents] skipping event that starts after it ends'
                );
            ...
            if (endTime - startTime > daySeconds) {
              if (pmd)
                console.log(
                  '[getScheduledEvents] skipping event that lasts more than 1 day'
                );
            ...
            if (!(title && location && startTime && endTime)) {
              if (pmd)
                console.log(
                  '[getScheduledEvents] skipping event that is missing one of title, location, startTime, endTime'
                );
              continue;
            }
            console.log(
              `[getScheduledEvents] adding a scheduled event: ${title} (${location} starting ${startTime})`
            );
            // all good to include
            let matchedEvent = {
              startTime,
          ...
          if (matchedEvents.length < scheduledEventLimit) {
            console.log(
              '[getScheduledEvents] checking live activities for planned events'
            );
            ...
            for (let event of liveActivitiesEvents) {
              ...
              for (let eventTag of eventTags) {
                ...
                if (eventTag[0] == 'status' && eventTag[1].length > 0) {
                  status = eventTag[1];
                }
              }
              // Check for deleted
              ...
              if (service == 'https://nostrnests.com') {
                ...
                let nostrnestsRelays = [
                  'wss://relay.snort.social/',
                  'wss://nos.lol/',
                  'wss://relay.damus.io/',
                  'wss://nostr.land/',
                ];
                let naddr1 = nip19.naddrEncode({
                  identifier: dTag,
                  pubkey: event.pubkey,
                  kind: event.kind,
                  relays: nostrnestsRelays,
                });
              ...
              console.log(
                `[getScheduledEvents] adding a live activity event: ${title} (${location} starting ${startTime})`
              );
              // all good to include
              let plannedActivity = {
                startTime,
        ...
        localpool.subscribe(
          liveactivitiesFilter,
          relaysToUse,
          (event, onEose, url) => {
            liveActivitiesEvents.push(event);
          },
          undefined,
          undefined,
          options
        ...
        rej(undefined);
        console.log(
          '[getScheduledEvents] There was an error while fetching scheduled events: ',
          error
        );
      }
    } catch (localpoolerror) {
      console.log(
        '[getScheduledEvents] error creating relay pool: ',
        localpoolerror
      );
    }
  });
...
const publishNostrSchedule = async (
  roomId,
  schedule,
  moderatorids,
  logoURI
  ...
  try {
    ...
    if (
      !schedule.setByNpub ||
      schedule.setByNpub == '' ||
      schedule.setByNpub.length != 63
    )
      return;

    console.log(
      '[publishNostrSchedule] Preparing schedule to publish for room',
      roomId
    );
    const localwritepool = getRelayPool();
    ...
    const content =
      (schedule?.summary ??
        `This event is scheduled on Corny Chat in room: ${roomId}`) +
      `\n\n${roomUrl}`;
    ...
    let scheduledRoomSet = await set(`scheduledRoom/${roomId}`, {
      repeat: schedule.repeat,
      start: schedule?.startUnixTime,
      end: schedule?.endUnixTime,
    });
    // inject into server/scheduledEvents
    let injectedSchedule = await injectServerSchedule({
      startTime: schedule?.startUnixTime,
      endTime: schedule?.endUnixTime,
      image: logoURI,
      location: roomUrl,
      title: title,
    });
    ...
    console.log(
      '[publishNostrSchedule] Preparing kind 1 to publish event from room'
    );
...
const ejectServerSchedule = async eventInfo => {
  ...
  try {
    ...
    for (let serverEvent of serverEvents) {
      if (
        serverEvent.location == eventInfo.location &&
        serverEvent.startTime == eventInfo.startTime
      ) {
        removed = true;
        continue;
      }
...
const filterEventsByLocation = events => {
  ...
  for (let event of events) {
    ...
    if (seenLocations.includes(seenLocation)) {
      console.log(
        `[filterEventsByLocation] filtering matched event with same location: ${event.title} (${event.location} starting ${event.startTime})`
      );
      continue;
    }
...
const updateNostrProfileForServer = async (
  name,
  description,
  logoURI,
  backgroundURI,
  ...
  try {
    ...
    if (pmd)
      console.log(
        '[updateNostrProfileForServer] Updating server profile from env'
      );
    const localwritepool = getRelayPool();
    const sk = nip19.decode(serverNsec).data;
...
const updateSupportedKinds = async () => {
  try {
    if (serverNsec.length == 0) return;
    if (pmd) console.log('[updateSupportedKinds] Publishing kind 31990 event');
    const localwritepool = getRelayPool();
    const sk = nip19.decode(serverNsec).data;
    const event = finalizeEvent(
      {
    ...
    console.log(`[updateSupportedKinds] Error ${e}`);
  }
};
...
const deleteLiveActivity = async (roomId, dtt, eventId) => {
  try {
    if (pmd)
      console.log(
        '[deleteLiveActivity] deleting known activity for room ',
        roomId
      );
    const localwritepool = getRelayPool();
    let roomNsec = await getRoomNSEC(roomId, true);
  ...
  return new Promise(async (res, rej) => {
    try {
      ...
      try {
        ...
        rej(undefined);
        console.log(
          '[getLiveActivities] error while fetching live activities: ',
          error
        );
      }
    } catch (localpoolerror) {
      console.log(
        '[GetLiveActivities] error creating relay pool: ',
        localpoolerror
      );
    }
  });
...
const publishLiveActivity = async (
  roomId,
  dtt,
  roomInfo,
  userInfo,
  ...
  try {
    ...
    const summary =
      ...
      `This is a live event on Corny Chat in room: ${roomId}`;
    ...
    if (pmd)
      console.log(
        '[publishLiveActivity] event to be published',
        JSON.stringify(event)
      );
    let publishToRelays =
      limitToRelays != undefined ? limitToRelays : relaysToUse;
    if (pmd)
      console.log(
        '[publishLiveActivity] to relays',
        JSON.stringify(publishToRelays)
      );
...
const publishRoomActive = async (roomId, dtt, roomInfo, userInfo, isnew) => {
  try {
    ...
    if (pmd)
      console.log(
        '[publishRoomActive] publishing room with live activity for ',
        roomId
      );
    const localwritepool = getRelayPool();
    const kind = 1;
...
const getZapGoals = async pubkey => {
  ...
  return new Promise(async (res, rej) => {
    try {
      ...
      console.log('[getZapGoals] error creating relay pool: ', localpoolerror);
    }
  });
};
...
const publishZapGoal = async (sk, content, amount, relays) => {
  ...
  return new Promise(async (res, rej) => {
    try {
      ...
      console.log(
        '[publishZapGoal] error creating relay pool: ',
        localpoolerror
      );
    }
  });
...
const deleteOldZapGoals = async sk => {
  try {
    ...
    if (el > 0) {
      if (pmd)
        console.log(
          `[deleteOldZapGoals] requesting deletion of ${el} zap goal events`
        );
      ...
      if (pmd)
        console.log(
          `[deleteOldZapGoals] no zap goal events need to be deleted`
        );
    }
    doneRelayPool(localwritepool);
  } catch (err) {
...
async function getNpubs(identityKeys) {
  ...
  for (let identityKey of identityKeys) {
    try {
      ...
      console.log(
        '[getNpubs] error in getNpubs conversion for identity: ',
        identityKey,
        error
      );
    }
