...
const serverProfileInitializer = async () => {
  ...
  if (serverNsec.length > 0) {
    ...
    let description =
      process.env.SERVER_PROFILE_ABOUT ||
      `This bot periodically posts open audio spaces for ${jamHost}`;
    let logoURI =
      process.env.SERVER_PROFILE_PICTURE ||
      `https://${jamHost}/img/cornychat-app-icon-512.png`;
    let backgroundURI =
      process.env.SERVER_PROFILE_BANNER ||
      `https://${jamHost}/img/cornychat-banner.png`;
    ...
    try {
      let result = await updateNostrProfileForServer(
        name,
        description,
        logoURI,
        backgroundURI,
      ...
      let result2 = await updateSupportedKinds();
    } catch (error) {
