...
const liveEventUpdater = async () => {
  ...
  try {
    ...
    console.log(
      `[liveEventUpdater] checking cache of prior live rooms on startup, and ending older events`
    );
    try {
      liveActivities = await getLiveActivities();
    } catch (e) {
      console.log(
        `[liveEventUpdater] error caching initial live rooms on startup: ${e}`
      );
    }
    if (liveActivities) {
      for (let relayUrl of Object.keys(liveActivities)) {
        ...
        if (pmd)
          console.log(
            `[liveEventUpdater] processing ${relayActivities.length} events on relay ${relayUrl}`
          );
        for (let oldLiveActivity of relayActivities) {
          ...
          if (!dTag) {
            if (pmd)
              console.log(
                `[liveEventUpdater] unable to end or delete live activity without d tag: ${JSON.stringify(
                  oldLiveActivity
                )}`
              );
            continue;
          ...
          if (!rTag) {
            if (pmd)
              console.log(
                `[liveEventUpdater] unable to end or delete live activity without r tag to identity room id: ${JSON.stringify(
                  oldLiveActivity
                )}`
              );
            continue;
          ...
          if (!lTag) {
            if (pmd)
              console.log(
                `[liveEventUpdater] skipping live activity that is not from this host: ${JSON.stringify(
                  oldLiveActivity
                )}`
              );
            continue;
          ...
          if (!status) {
            if (pmd)
              console.log(
                `[liveEventUpdater] unable to end or delete live activity without status tag: ${JSON.stringify(
                  oldLiveActivity
                )}`
              );
            continue;
          ...
          if (status == 'ended') {
            ...
            if (endTime) {
              if (
                ...
                if (pmd)
                  console.log(
                    `[liveEventUpdater] deleting event that is older than 1 day`
                  );
                try {
                  let dla = await deleteLiveActivity(key, dtt, eventId);
                } catch (edla) {
                  // ignore it and hope it works next time
                  // TODO: cache signed message and retry?
                }
              } else {
                if (pmd)
                  console.log(
                    `[liveEventUpdater] skipping delete. time is ${Math.floor(
                      Date.now() / 1000
                    )}, and ends is ${Math.floor(endTime)}`
                  );
              }
            } else {
              if (pmd)
                console.log(
                  `[liveEventUpdater] skipping delete. it's ended, but no ends time is set`
                );
            }
          } else if (status == 'live') {
            ...
            if (!roomInfo || roomInfo == undefined) {
              if (pmd)
                console.log(
                  `[liveEventUpdater] ignoring event for room ${key} as it doesnt exist here`
                );
              continue;
            }
            ...
            if (!activeRoomTimes.hasOwnProperty(key)) {
              if (pmd)
                console.log(
                  `[liveEventUpdater] tracking as active room ${key} with dTag: ${dTag}`
                );
              ...
              if (pmd)
                console.log(
                  `[liveEventUpdater] ending older event for room ${key} and dtt ${dtt} on ${relayUrl}`
                );
              let userInfo = [];
              if (pmd)
                console.log(
                  `[liveEventUpdater] event to be replaced on ${relayUrl}`,
                  JSON.stringify(oldLiveActivity)
                );
              let pla = await publishLiveActivity(
                key,
                dtt,
                roomInfo,
                userInfo,
    ...
    try {
      ...
      for (let roomId of roomIds) {
        ...
        let userInfo = await Promise.all(
          peerIds.map(id => get(`identities/${id}`))
        );
        ...
        let isLiveActivityAnnounced =
          roomInfo?.isLiveActivityAnnounced ?? false;
        if (!isLiveActivityAnnounced || isPrivate || isClosed) {
          if (pmd)
            console.log(
              `[liveEventUpdater] room is no longer live activity, or is private or closed`
            );
          ...
          if (runCounter % liveActivitiesUpdateInterval == 1 || isnew) {
            if (pmd)
              console.log(
                `[liveEventUpdater] about to ${
                  isnew ? 'create new' : 'update'
                } live activity for ${roomId}`
              );
            let pla = await publishLiveActivity(
              roomId,
              dttr,
        ...
        if (!isPrivate && !isClosed) {
          ...
          if (pmd)
            console.log(
              `[liveEventUpdater] checking if need to publish room ${roomId} as active`
            );
          if (activeRoomTimesAnnounced.hasOwnProperty(roomId))
            announceit = activeRoomTimesAnnounced[roomId] < three_hours_ago;
          if (announceit) {
            activeRoomTimesAnnounced[roomId] = dtt;
            if (pmd)
              console.log(
                `[liveEventUpdater] publishing ${roomId} as active in kind 1 note`
              );
            let pk1 = await publishRoomActive(
              roomId,
              dttr,
              roomInfo,
              userInfo,
            ...
            if (pmd)
              console.log(
                `[liveEventUpdater] skipping announcement for ${roomId}, was previously published at ${activeRoomTimesAnnounced[roomId]}`
              );
          }
        }
      }
      ...
      Object.keys(activeRoomTimes).forEach(key => {
        if (!roomsWithUsers.includes(key)) {
          ...
          (async () => {
            ...
            let roomInfo = await get(roomKey);
            if (pmd)
              console.log(`[liveEventUpdater] ending live activity for ${key}`);
            let pla = await publishLiveActivity(
              key,
              dtt,
              roomInfo,
